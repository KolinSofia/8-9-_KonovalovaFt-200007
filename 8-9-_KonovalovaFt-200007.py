...
def readCoordinates(chess_xy):
    ...
    while check == 0:
        try:
            chess_xy[0][0],chess_xy[0][1] = map(int, input("Координаты первого поля числами от 1 до 8: ").split())
        except Exception:
    ...
    while check == 0:
        try:
            chess_xy[1][0],chess_xy[1][1] = map(int, input("Координаты второго поля числами от 1 до 8: ").split())
        except Exception:
            print("Координаты не являются целыми числами")
            logging.error("Координаты не являются целыми числами")
            check = 0
...
def Bishop2Move(a):
    ...
    if diagonalCheck(a) == False:
        if colorCheck(a) == "не одного цвета": #cлон сможет попасть на клетку только своего цвета
            ...
            # проходимся по диагонали в одном направлении, пока не достигнем верхний/нижний предел для каждой координаты
            # если проверка на диагональность работает
            while x>0 and y>0:
                x -= 1
                y -= 1
